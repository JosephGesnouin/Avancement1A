...
Selon notre problématique: lier la dynamique de la pose à une classe, nous nous sommes majoritairement intéressés à la dernière famille d'approches. La représentation sous forme de squelette qui semble être l'approche la plus robuste mais également la plus plausible pour une utilisation en temps réel:
...
Il est dans ce cas alors possible d'utiliser des approches de reconnaissance d'actions pour l'inférence à court terme et ce même si elles ne sont pas optimisées pour la reconnaissance d'actions partielles. Cependant, ces méthodes ne sont pas suffisantes puisque non optimisées pour le problème donné. \cite{ryoo2011human} est la première publication mentionnant le principe de prédiction et reconnait les actions en cours de manière probabiliste en observant certaines features accumulés au fil du temps, d'autres approches \textit{handcrafted} telles que \cite{soomro2018online} et \cite{soomro2016predicting} utilisent des SVM binaires pour labéliser des extraits vidéo comme des catégories de sous-actions puis obtiennent le label en final en utilisant la programmation dynamique.\\
...
La durée de ces actions complexes est bien plus longue et leur prédiction en est plus compliquée que pour des actions à court terme. Il \\
