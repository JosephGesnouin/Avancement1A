...
L'apprentissage géométrique profond (\textit{Geometric deep learning}\cite{bronstein2017geometric}) est un terme générique désignant les techniques qui tentent de généraliser les réseaux de neurones profonds (structurés) à des domaines non euclidiens tels que les graphes.

La nature non euclidienne des données rend l'utilisation des opérations de base (convolutions) assez difficile à réaliser. L'apprentissage approfondi de la géométrie traite en ce sens de l'extension des techniques d'apprentissage approfondi aux graphiques

\textit{ En changeant complètement le type de structure de données utilisées, on change les problématiques :
	On ne travaille plus dans un environnement euclidien, les CNN ne sont donc plus applicables si l’on souhaite travailler sous format graphe. 
	Surcouche des CNN : Les CNN ont la capacité d'extraire des caractéristiques spatiales locales. Les graphes peuvent être intéressants puisque ils sont par définition, des structures connectées localement (l’ensemble des voisins d’un nœud). Certains travaux présentent leur méthodes de Graph Neural Network comme une généralisation des CNN.
	Petits graphes (15-30 nœuds par personne) : complexité maitrisée plus facilement, ce qui peut être intéressant une fois que l’on souhaite faire du temps réel, mais ce n’est qu’une supposition. De mémoire les algorithmes type graphe restent des méthodes dont la complexité en terme de temps/coût reste la plus optimisée, je peux dire une bêtise.
	Des versions temporelles des graphs neural network existent.}
