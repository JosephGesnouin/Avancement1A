...
Sur la base de la sous-partie sus-jacente, nous pouvons conclure que pour l'estimation de pose en 2D, les méthodes de l'état de l'art obtiennent de très bonnes performances grâce aux réseaux convolutifs.
...
Les méthodes d'estimation de poses en 3D sont bien moins matures que pour la 2D. Une des raisons principale à ce jour serait le manque de jeux de données fiables disponibles.
On peut cependant cerner deux types d'approches: premièrement estimer une pose en 2D et ensuite reconstruire une pose en 3D ou alors effectuer directement la regression d'une pose en 3D.

\cite{tekin2016structured} proposent par exemple une architecture de regression 3D. Au lieu d'utiliser directement un réseau convolutif pour estimer une pose 3D (le label), ceux-ci entrainent un auto-encoder espace latent sparse de dimension superieure à la dimension de l'entrée, ceci afin de capturer de la sémantique suplémentaire comparé aux seules informations présentes dans le label\\le squelette. Puis regressent grâce à un réseau convolutif prenant en entrée une image, les valeurs de l'espace latent pour cette instance et utilisent la partie décodeur afin de récupérer la pose en 3D.

\cite{rogez2019lcr} proposent de découper le problème d'estimation de poses 3D en estimant simultanément les poses en 2D et en 3D et en combinant les résultats obtenus.

\cite{singleshotmultiperson2018} proposent une  méthode de regression réutilisant la même approche bottom-up que \cite{cao2017realtime} pour associer les keypoints, cependant proposent d'utiliser des \textit{occlusion-robust pose-map} (ORPM), permettant d'inférer la pose du corps entier même en cas de fortes occlusions partielles par d'autres personnes ou d'objets.
